...
async function run() {
  try {
    ...
    const assetId =
      shuffleRes.availableAssets[
        Math.floor(Math.random() * shuffleRes.availableAssets.length)
      ].index;
    const fields = {
      reservedAssets: [],
      availableAssets: [],
    ...
    for (const asset of shuffleRes.availableAssets) {
      asset.index === assetId
        ? fields.reservedAssets.push(asset)
        : fields.availableAssets.push(asset);
    }
